//Ejercicio 36
//Mauricio Abbati Loureiro - EDA 2ºE 2018/2019
//E01
#include <iostream>
#include <fstream>
#include <string>
#include "ambulatorio.h"


bool resuelveCaso() {
    
    int i = 0;
    int operaciones;
    
    std::cin >> operaciones;
    if (!std::cin)
        return false;
    
    std::string orden;
    medico m;
    Fecha f;
    paciente p;
    bool escrito = false;
    
    ambulatorio hospitalDeLosHorrores;
    
    while (i < operaciones) {
        try {
            std::cin >> orden;
            if (orden == "nuevoMedico") {
                std::cin >> m;
                hospitalDeLosHorrores.nuevoMedico(m);
            }
            else if (orden == "pideConsulta") {
                int dia, hora, minuto;
                std::cin >> p >> m >> dia >> hora >> minuto;
                f = Fecha(dia, hora, minuto);
                hospitalDeLosHorrores.pideConsulta(p, m, f);
            }
            else if (orden == "listaPacientes") {
                int dia;
                std::cin >> m;
                std::cin >> dia;
                std::vector<tCita> l = hospitalDeLosHorrores.listaPacientes(m, dia);
                std::cout << "Doctor " << m << " " << "dia " << dia << "\n";
                for (auto &p : l){
                    std::cout << p.p << " ";
                    p.f.pintarHora();
                    std::cout << "\n";
                }
                escrito = true;
            }
            else if (orden == "siguientePaciente") {
                std::cin >> m;
                p = hospitalDeLosHorrores.siguientePaciente(m);
                std::cout << "Siguiente paciente doctor " << m << "\n" << p << "\n";
                escrito = true;
            }
            else if (orden == "atiendeConsulta"){
                std::cin >> m;
                hospitalDeLosHorrores.atiendeConsulta(m);
            }
        }
        catch (std::domain_error e) {
            std::cout << e.what() << "\n";
            escrito = true;
        }
        if (escrito) std::cout << "---\n";
        escrito = false;
        ++i;
    }
    std::cout << "------\n";
    
    return true;
}

int main() {
    
    // Para la entrada por fichero.
    // Comentar para acepta el reto
#ifndef DOMJUDGE
    std::ifstream in("datos.txt");
    auto cinbuf = std::cin.rdbuf(in.rdbuf()); //save old buf and redirect std::cin to casos.txt
#endif
    
    while (resuelveCaso());
    
    // Para restablecer entrada. Comentar para acepta el reto
#ifndef DOMJUDGE // para dejar todo como estaba al principio
    std::cin.rdbuf(cinbuf);
    //system("PAUSE");
#endif
    
}
